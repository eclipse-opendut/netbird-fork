...
func NewAuth(ctx context.Context, privateKey string, mgmURL *url.URL, config *profilemanager.Config) (*Auth, error) {
	...
	mgmClient, err := mgm.NewClient(ctx, mgmURL.Host, myPrivateKey, mgmTLSEnabled, mgm.WithClientCert(config.MgmtClientCert.KeyPair))
	if err != nil {
...
func (a *Auth) reconnect(ctx context.Context, brokenClient *mgm.GrpcClient) error {
	...
	mgmClient, err := mgm.NewClient(ctx, a.mgmURL.Host, a.privateKey, a.mgmTLSEnabled, mgm.WithClientCert(a.config.MgmtClientCert.KeyPair))
	if err != nil {
